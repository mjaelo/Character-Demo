...
public partial class PlayerCameraManager : Node3D
{
	...
	public override void _Ready()
	{
		...
		_camera3P = GetNode<Camera3D>("SpringArm3D/3PCamera");
		CurrentCamera = _camera3P;
		_camera3P.Current = true;
	...
	public override void _Input(InputEvent @event)
	{
		switch (@event)
		{
			...
			case InputEventMouseButton { ButtonIndex: MouseButton.WheelDown }:
				if (CurrentCamera == _camera3P) Camera.Zoom(MobConstants.ZoomStepMouseGame);
	...
	private void SwitchCameras()
	{
		...
		CurrentCamera = CurrentCamera == _camera3P ? Camera1P : _camera3P;
		CurrentCamera.Current = true;

		_mob.Scale += CurrentCamera == _camera3P ? new Vector3(0, -0.15f, 0) : new Vector3(0, 0.15f, 0);
		foreach (var (k, v) in _headMeshVisibility)
			_mob.GetNode<MeshInstance3D>("body/Armature/Skeleton3D/" + k).Visible = CurrentCamera == _camera3P && v;
