...
namespace CharacterDemo.UI.Scenes.CreatorScenes.PresetTabScene;
...
public partial class PresetTab : TabBar
{
	...
	private void OnPresetChanged(string mobName)
	{
		...
		if (!isNew && _presets.TryGetValue(mobName, out var md))
		{
			...
			MobDataSetter.SetMobDataToMob(newMobData, _parent.Player);
	...
	{
		_parent.UpdatePrevMobData();
		var norms = MobDataGetter.GetRtgNorms(_parent.MobData.Race, _parent.MobData.Type, _parent.MobData.Gender);
		_parent.MobData.BodyData = MobDataGetter.GetRandomBodyData(norms);
		MobDataSetter.SetBodyData(_parent.MobData.BodyData, _parent.Player);
		_parent.SetMobDataToPickers(_parent.MobData);
	}
	...
	private void OnRandomClothesPressed()
	{
		...
		_parent.MobData.EquipmentData = MobDataGetter.GetRandomEquipmentData(norms, null,skinColor);
		MobDataSetter.SetEquipmentData(_parent.MobData.EquipmentData, _parent.Player);
		_parent.SetMobDataToPickers(_parent.MobData);
	}
