...
namespace CharacterDemo.UI.Scenes.CreatorScenes;
...
public class TabBuilder(Skeleton3D skeleton, Creator creator)
{
        ...
        MeshInstance3D? meshInstance, string shapeName)
    {
        ...
        int shapeId = MobGetter.GetShapeNamesFromMesh(meshInstance.Mesh).ToList().IndexOf(shapeName);
    ...
    private void OnMeshPickerChanged(string value, MeshInstance3D meshInstance, string meshName, string fileFolder)
    {
        ...
        MeshData? meshData = MobConstants.BodyMeshesInfo.TryGetValue(meshName, out var bmi)
            ? MobDataGetter.GetBodyDataFieldValue(creator.MobData.BodyData, bmi.FieldName)
            : MobConstants.EqMeshesInfo.TryGetValue(meshName, out var emi)
                ? MobDataGetter.GetEqDataFieldValue(creator.MobData.EquipmentData, emi.FieldName)
                : null;
        if (meshData == null) return;
        
        meshData.MeshFile = value;
        var color = meshData.MeshColors[MobGetter.GetMainMaterial(meshInstance)];
        
        MobSetter.SetMeshFile(value, meshInstance, fileFolder);
        MobSetter.SetMeshColor(color, meshInstance);
    }
    ...
    private void OnColorPickerChanged(Color value, MeshInstance3D meshInstance, string meshName, int materialNr = -1)
    {
        ...
        {
            var meshData = MobDataGetter.GetBodyDataFieldValue(creator.MobData.BodyData, bmi.FieldName);
        ...
        {
            var meshData = MobDataGetter.GetEqDataFieldValue(creator.MobData.EquipmentData, emi.FieldName);
            var newColors = meshData.MeshColors.ToList();
            while (newColors.Count <= materialNr) newColors.Add(new Color());
            newColors[materialNr] = value;
            meshData.MeshColors = newColors;
        ...
        MobSetter.SetMeshColor(value, meshInstance, materialNr);
        ...
        string shapeName)
    {
        ...
        if (MobConstants.BodyMeshesInfo.TryGetValue(meshName, out var bmi))
        {
            var bodyField = MobDataGetter.GetBodyDataFieldValue(creator.MobData.BodyData, bmi.FieldName);
        ...
        {
            var eqField = MobDataGetter.GetEqDataFieldValue(creator.MobData.EquipmentData, emi.FieldName);
        ...
        MobSetter.SetSkeletonShapeKey(value, shapeName, (Skeleton3D)meshInstance.GetParent());
    }
}
