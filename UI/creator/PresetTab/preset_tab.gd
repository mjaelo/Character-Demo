...
@onready var preset_picker := $ScrollContainer/VBoxContainer/Preset
@onready var mob_type_picker := $ScrollContainer/VBoxContainer/MobType
@onready var mob_race_picker := $ScrollContainer/VBoxContainer/Race
...
var def_name := "New Character"
var presets := {def_name: {}}

func _ready():
	# setup preset picker
	var saved_presets = FileUtils.serialize_and_load("preset.json","res://UI/creator/")
	for key in saved_presets.keys():
		presets[key] = saved_presets[key]
	
	var first_element = {"key1": "value1"}
	var given_dict = {"key2": "value2", "key3": "value3"}
	for key in given_dict.keys():
		first_element[key] = given_dict[key]
