...
static func set_body_data(body_data:BodyData, mob:Mob):
	...
	for mesh_name:String in MobConstants.BODY_MESHES_INFO.keys():
		...
		var mesh_data: MeshData = body_data[field_name]
...
static func set_equipment_data(eq_data:EquipmentData, mob:Mob):
	...
	for mesh_name:String in MobConstants.EQ_MESHES_INFO.keys():
		...
		var field_name: String = mesh_info.field_name
		var mesh_data: MeshData = eq_data[field_name]
		var mesh_instance: MeshInstance3D = MobUtils.get_mesh_from_skeleton(mesh_name, skeleton)
		if mesh_instance:
			set_mesh_data(mesh_data, mesh_name, mesh_instance)
...
static func set_mesh_data(mesh_data:MeshData, mesh_name:String, mesh_instance:MeshInstance3D):
	if mesh_data.mesh_file:
		var is_body_mesh := MobConstants.BODY_MESHES_INFO.has(mesh_name)
		var mesh_info: MobMeshInfo = MobConstants.BODY_MESHES_INFO[mesh_name] if is_body_mesh else MobConstants.EQ_MESHES_INFO[mesh_name]
